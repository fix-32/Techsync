...
async (conn, mek, m, { from, body, isOwner }) => {
    const filePath = path.join(__dirname, '../../Techsync/data/autovoice.json');
...
async (conn, mek, m, { from, body, isOwner }) => {
    const filePath = path.join(__dirname, '../../Techsync/data/autosticker.json');
    const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    for (const text in data) {
        if (body.toLowerCase() === text.toLowerCase()) {
...
async (conn, mek, m, { from, body, isOwner }) => {
    const filePath = path.join(__dirname, '../../Techsync/data/autoreply.json');
    const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    for (const text in data) {
        if (body.toLowerCase() === text.toLowerCase()) {
